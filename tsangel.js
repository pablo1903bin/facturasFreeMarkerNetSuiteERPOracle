  ...
  function onRequest(option, context) {
    ...
    function star() {
      ...
      try {
        ...
        try {
          temp[0].saldo = curFormatter.format({
            number: parseFloat(temp[0].saldo),
          });
        } catch (ex) {
          temp[0].saldo = "$ 000.00";
        }
        try {
          temp[0].saldo1 = curFormatter.format({
            number: parseFloat(temp[0].saldo1),
          });
        } catch (ex) {
          temp[0].saldo1 = "$ 000.00";
        }
        try {
          temp[1].saldo = curFormatter.format({
            number: parseFloat(temp[1].saldo),
          });
        } catch (ex) {
          temp[1].saldo = "$ 000.00";
        }
        try {
          temp[1].saldo1 = curFormatter.format({
            number: parseFloat(temp[1].saldo1),
          });
        } catch (ex) {
          temp[1].saldo1 = "$ 000.00";
        }
        try {
          temp[2].saldo = curFormatter.format({
            number: parseFloat(temp[2].saldo),
          });
        } catch (ex) {
          temp[2].saldo = "$ 000.00";
        }
        try {
          temp[2].saldo1 = curFormatter.format({
            number: parseFloat(temp[2].saldo1),
          });
        } catch (ex) {
          temp[2].saldo1 = "$ 000.00";
        }
        try {
          temp[3].saldo = curFormatter.format({
            number: parseFloat(temp[3].saldo),
          });
        } catch (ex) {
          temp[3].saldo = "$ 000.00";
        }
        try {
          temp[3].saldo1 = curFormatter.format({
            number: parseFloat(temp[3].saldo1),
          });
        } catch (ex) {
          temp[3].saldo1 = "$ 000.00";
        }
        try {
          temp[4].saldo = curFormatter.format({
            number: parseFloat(temp[4].saldo),
          });
        } catch (ex) {
          temp[4].saldo = "$ 000.00";
        }
        try {
          temp[4].saldo1 = curFormatter.format({
            number: parseFloat(temp[4].saldo1),
          });
        } catch (ex) {
          temp[4].saldo1 = "$ 000.00";
        }
        try {
          temp[5].saldo = curFormatter.format({
            number: parseFloat(temp[5].saldo),
          });
        } catch (ex) {
          temp[5].saldo = "$ 000.00";
        }
        try {
          temp[5].saldo1 = curFormatter.format({
            number: parseFloat(temp[5].saldo1),
          });
        } catch (ex) {
          temp[5].saldo1 = "$ 000.00";
        }
        try {
          temp[6].saldo = curFormatter.format({
            number: parseFloat(temp[6].saldo),
          });
        } catch (ex) {
          temp[6].saldo = "$ 000.00";
        }
        try {
          temp[6].saldo1 = curFormatter.format({
            number: parseFloat(temp[6].saldo1),
          });
        } catch (ex) {
          temp[6].saldo1 = "$ 000.00";
        }
        try {
          temp[9].saldo = curFormatter.format({
            number: parseFloat(temp[9].saldo),
          });
        } catch (ex) {
          temp[9].saldo = "$ 000.00";
        }
        try {
          temp[9].saldo1 = curFormatter.format({
            number: parseFloat(temp[9].saldo1),
          });
        } catch (ex) {
          temp[9].saldo1 = "$ 000.00";
        }

        //dias
        if (isNaN(parseInt(Math.round(temp[7].saldo)))) {
          temp[7].saldo = " 0";
        } else {
          temp[7].saldo = parseInt(Math.round(temp[7].saldo));
        }
        if (isNaN(parseInt(Math.round(temp[7].saldo1)))) {
          temp[7].saldo1 = " 0";
        } else {
          temp[7].saldo1 = parseInt(Math.round(temp[7].saldo1));
        }
        if (isNaN(parseInt(Math.round(temp[8].saldo)))) {
          temp[8].saldo = " 0";
        } else {
          temp[8].saldo = parseInt(Math.round(temp[8].saldo));
        }
        if (isNaN(parseInt(Math.round(temp[8].saldo1)))) {
          temp[8].saldo1 = " 0";
        } else {
          temp[8].saldo1 = parseInt(Math.round(temp[8].saldo1));
        }

        //Variacion
        if (isNaN(parseFloat(temp[0].variacion))) {
          temp[0].variacion = " 0.0";
        } else {
          temp[0].variacion = parseFloat(temp[0].variacion);
        }
        if (isNaN(parseFloat(temp[1].variacion))) {
          temp[1].variacion = " 0.0";
        } else {
          temp[1].variacion = parseFloat(temp[1].variacion);
        }
        if (isNaN(parseFloat(temp[2].variacion))) {
          temp[2].variacion = " 0.0";
        } else {
          temp[2].variacion = parseFloat(temp[2].variacion);
        }
        if (isNaN(parseFloat(temp[3].variacion))) {
          temp[3].variacion = " 0.0";
        } else {
          temp[3].variacion = parseFloat(temp[3].variacion);
        }
        if (isNaN(parseFloat(temp[4].variacion))) {
          temp[4].variacion = " 0.0";
        } else {
          temp[4].variacion = parseFloat(temp[4].variacion);
        }
        if (isNaN(parseFloat(temp[5].variacion))) {
          temp[5].variacion = " 0.0";
        } else {
          temp[5].variacion = parseFloat(temp[5].variacion);
        }
        if (isNaN(parseFloat(temp[6].variacion))) {
          temp[6].variacion = " 0.0";
        } else {
          temp[6].variacion = parseFloat(temp[6].variacion);
        }
        if (isNaN(parseFloat(temp[7].variacion))) {
          temp[7].variacion = " 0.0";
        } else {
          temp[7].variacion = parseFloat(temp[7].variacion);
        }
        if (isNaN(parseFloat(temp[8].variacion))) {
          temp[8].variacion = " 0.0";
        } else {
          temp[8].variacion = parseFloat(temp[8].variacion);
        }
        if (isNaN(parseFloat(temp[9].variacion))) {
          temp[9].variacion = " 0.0";
        } else {
          temp[9].variacion = parseFloat(temp[9].variacion);
        }
        ...
        for (
          var i = 0;
          i < 20;
          i++ //al arreglo temp[] le da fotma decimal
        ) {
      ...
      try {
        ...
        try {
          ...
          for (var i = 0; i < results.length; i++) {
            ...
            indicadores.push({
              id: result.id,
              Indicador: result.getValue("custrecord_indicador"),
              clasificacion: result.getValue("custrecord_tipo"),
              busqueda: result.getValue("custrecord_busqueda"),
              funcion: result.getValue("custrecord_funcion"),
            });
          ...
          log.error({
            title:
              "Eror al Leer Configuracion - Agregar Reultados de la Busqueda",
            details: error,
          });
      ...
      if (tabla.length > 0) {
        for (var i = 0; i < tabla.length; i++) {
          ...
          if (etiqueta == "Egreso promedio diario" && i == 9) {
            ...
            if (isNaN(parseFloat(matriz[1].saldo1))) {
              monto1 = parseFloat(0);
            } else {
              parseFloat(matriz[1].saldo1);
            }
            var monto2 = matriz[2].saldo1;
            if (isNaN(parseFloat(matriz[2].saldo1))) {
              monto2 = parseFloat(0);
            } else {
              parseFloat(matriz[2].saldo1);
            }
            var monto3 = matriz[3].saldo1;
            if (isNaN(parseFloat(matriz[3].saldo1))) {
              monto3 = parseFloat(0);
            } else {
              parseFloat(matriz[3].saldo1);
            }
            var monto4 = matriz[4].saldo1;
            if (isNaN(parseFloat(matriz[4].saldo1))) {
              monto4 = parseFloat(0);
            } else {
              parseFloat(matriz[4].saldo1);
            }
            var monto5 = matriz[5].saldo1;
            if (isNaN(parseFloat(matriz[5].saldo1))) {
              monto5 = parseFloat(0);
            } else {
              parseFloat(matriz[5].saldo1);
            }
            var monto6 = matriz[6].saldo1;
            if (isNaN(parseFloat(matriz[6].saldo1))) {
              monto6 = parseFloat(0);
            } else {
              parseFloat(matriz[6].saldo1);
            }
            ...
            matriz.push({
              ...
              tipo: tabla[i].clasificacion,
              etiqueta: tabla[i].Indicador,
              busqueda: tabla[i].busqueda,
              funcion: tabla[i].funcion,
            });
            i++;
            continue;
            //}
          ...
          if (dif[0] == "Saldo") {
            if (dif[1] == "dias") {
              ...
              if (monto / dias3 === Infinity) {
                monto = 0.0;
              } else {
                monto = monto / dias3;
              }
              if (monto1 / dias4 === Infinity) {
                monto1 = 0.0;
              } else {
                monto1 = monto1 / dias4;
              }
              ...
              if (filtro.length == 1) {
                ...
                if (monto / dia === Infinity) {
                  monto = 0.0;
                } else {
                  monto = monto / dia;
                }
                if (monto1 / dia1 === Infinity) {
                  monto1 = 0.0;
                } else {
                  monto1 = monto1 / dia1;
                }
...
*/
    ...
    function fechaadd(fecha, fechafin, mes) {
      ...
      try {
        ...
        if (arrg2[1] == 2 && biciesto(f.getFullYear()) == true) {
      ...
      try {
        ...
        if (actual < anterior) {
          if (actual == 0 && anterior > 0) {
            ...
            valor = (((anterior - actual) / anterior) * 100 * -1)
              .toFixed(2)
              .toString();
          }
        } else {
    ...
    function Formulario(lista, fechain, fechafin) {
      ...
      if (filtro.length > 0) {
        var sublist = form.addSublist({
          id: "custpage_indicadores",
          type: ui.SublistType.LIST,
          label: "Primario",
        });
        sublist.addField({
          id: "custpage_indicador",
          type: ui.FieldType.TEXT,
          label: "Indicador",
        });
        sublist.addField({
          id: "custpage_encabezadoact",
          type: ui.FieldType.TEXT,
          label: fec[3].toString(),
        });
        sublist.addField({
          id: "custpage_encabezadoant",
          type: ui.FieldType.TEXT,
          label: fec[4].toString(),
        });
        sublist.addField({
          id: "custpage_encabezadoporcentual",
          type: ui.FieldType.TEXT,
          label: "VARIACIÓN",
        });
        ...
        for (var j = 0; j < filtro.length; j++) {
          sublist.setSublistValue({
            id: "custpage_indicador",
            line: linea,
            value: filtro[j].etiqueta,
          });
          sublist.setSublistValue({
            id: "custpage_encabezadoact",
            line: linea,
            value: filtro[j].saldo,
          });
          sublist.setSublistValue({
            id: "custpage_encabezadoant",
            line: linea,
            value: filtro[j].saldo1,
          });
          sublist.setSublistValue({
            id: "custpage_encabezadoporcentual",
            line: linea,
            value: filtro[j].variacion + " %",
          });
          linea++;
        }
      }
      ...
      if (filtro.length > 0) {
        var sublist = form.addSublist({
          id: "custpage_indicadores2",
          type: ui.SublistType.LIST,
          label: "Secundario",
        });
        sublist.addField({
          id: "custpage_indicador2",
          type: ui.FieldType.TEXT,
          label: "Indicador",
        });
        sublist.addField({
          id: "custpage_encabezadoact2",
          type: ui.FieldType.TEXT,
          label: fec[3].toString(),
        });
        sublist.addField({
          id: "custpage_encabezadoant2",
          type: ui.FieldType.TEXT,
          label: fec[4].toString(),
        });
        sublist.addField({
          id: "custpage_encabezadoporcentual2",
          type: ui.FieldType.TEXT,
          label: "VARIACIÓN",
        });
      ...
      if (filtro.length > 0) {
        var sublist = form.addSublist({
          id: "custpage_indicadores3",
          type: ui.SublistType.LIST,
          label: "Auxiliar",
        });
        sublist.addField({
          id: "custpage_indicador3",
          type: ui.FieldType.TEXT,
          label: "Indicador",
        });
        sublist.addField({
          id: "custpage_encabezadoact3",
          type: ui.FieldType.TEXT,
          label: fec[3].toString(),
        });
        sublist.addField({
          id: "custpage_encabezadoant3",
          type: ui.FieldType.TEXT,
          label: fec[4].toString(),
        });
        sublist.addField({
          id: "custpage_encabezadoporcentual3",
          type: ui.FieldType.TEXT,
          label: "VARIACIÓN",
        });
